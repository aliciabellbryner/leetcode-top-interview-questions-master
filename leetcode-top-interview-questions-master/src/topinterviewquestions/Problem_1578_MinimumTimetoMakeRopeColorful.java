 ...
 */
public class Problem_1578_MinimumTimetoMakeRopeColorful {
    ...
    public int minCost(String s, int[] cost) {
        ...
        for (int i = 0; i < n; ++i) {
            if (i > 0 && s.charAt(i) != s.charAt(i - 1)) {
                res += sum_cost - max_cost;//其实就相当于一组相同的chars，只能留一个最大的，这样花费的时间最少，所以累加上sum_cost - max_cost即可
                sum_cost = max_cost = 0;//把这两个值都变成0
            }
            sum_cost += cost[i];
