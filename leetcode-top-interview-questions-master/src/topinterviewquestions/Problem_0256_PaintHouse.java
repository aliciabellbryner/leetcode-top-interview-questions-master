...
1 <= costs[i][j] <= 20
 */
public class Problem_0256_PaintHouse {
    /*
    Solution
...
All the possible sequences and their costs.
...
Space complexity : Anywhere from O(n)O(n) to O(n \cdot 3^n)O(n⋅3
n
 ).

This would depend entirely on the implementation. If you generated all the permutations at the same time and put them in a massive list, then you'd be using O(n * 2^n)O(n∗2
n
 ) or O(n * 3^n)O(n∗3
n
 ) space. If you generated one, processed it, generated the next, processed it, etc, without keeping the long list, it'd require O(n)O(n) space.
...
Should we pick the green house or the blue house?
...
The cost of painting the first house red is the cost of that paint + the minimum out of painting the second house green or painting it blue.
...
While this approach is an improvement on the previous approach, it still requires exponential time. Think about the number of leaf nodes. Each permutation has its own leaf node. The number of internal nodes is the same as the number of leaf nodes too. Remember how there are 2^n2
n
  different permutations? Each effectively adds 2 nodes to the tree, so dropping the constant of 2 gives us O(2^n)O(2
n
 ).

This is better than the previous approach, which had an additional factor of n, giving O(n \cdot 2 ^ n)O(n⋅2
n
 ). That extra factor of n has disappeared here because the permutations are now "sharing" their similar parts, unlike before. The idea of "sharing" similar parts can be taken much further for this particular problem, as we will see with the remaining approaches that knock the time complexity all the way down to O(n)O(n).
...
The tree with one layer collapsed.
...
The tree with two layers collapsed.
...
In Python, we can use the lru_cache decorator from the functools package. If you're not familar with this, you can find it In the python documentation. It's very useful!
...
How the tree would look if collapsed.
...
We don't need to do anything to the last row.
...
The calculation for painting the second-to-last house red.
...
Finding the minimum of two values and adding it to another value is an O(1)O(1) operation. We are doing these O(1)O(1) operations for 3 \cdot (n - 1)3⋅(n−1) cells in the grid. Expanding that out, we get 3 \cdot n - 33⋅n−3. The constants don't matter in big-oh notation, so we drop them, leaving us with O(n)O(n).

A word of warning: This would not be correct if there were mm colors. For this particular problem we were told there's only 33 colors. However, a logical follow-up question would be to make the code work for any number of colors. In that case, the time complexity would actually be O(n \cdot m)O(n⋅m), because mm is not a constant, whereas 33 is. If this confused you, I'd recommend reading up on big-oh notation.
...
It's up to you whether you do this using length-3 arrays or variables. Arrays are better in terms of writing clean code though. They will also be easier to adapt if you were asked to make the algorithm work with mm colors. I have chosen to use arrays here as keeping track of 6 seperate variables is too messy.
...
Problems that have optimal substructure can be solved with greedy algorithms. If they also have overlapping subproblems, then they can be solved with dynamic programming algorithms.
    ...
    //wrong solution, Thanks so much to @bitbleach for pointing out that the original code I had here was over writing the input array! Because this is such an easy mistake to make, I've kept the original code for reference.
    class Solution5_wrong {
        public int minCost(int[][] costs) {
    ...
    //diss Here is my solution without modifying the input array and with constant space
    ...
    //Read More
    //The DP idea is not quite straightforward if you are seeing this question for the 1st time. Here dp[i][j] means the minimum cost of painting houses from [0, i] if we paint ith house with color j. Because j can only be 3 colors - 0, 1, 2, if j==0, dp[i][0] = costs[i][0] + Math.min(the min cost of painting (i-1)th house with either blue or green) = costs[i][0] + Math.min(dp[i-1][1], dp[i-1][2]). Changing input is definitely a bad practice and it's not worth doing it in this question. Below is my solution using two arrays of constant length.
    public int minCost(int[][] costs) {
        if(costs.length==0) return 0;
        int[] pre = new int[3];
        int[] cur = new int[3];
