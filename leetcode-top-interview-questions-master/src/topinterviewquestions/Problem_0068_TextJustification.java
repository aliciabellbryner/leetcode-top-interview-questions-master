...
public class Problem_0068_TextJustification {
	...
	//rightMost: Then we greedily try to go as far right as possible until we fill our current line.
	...
	public List<String> fullJustify(String[] words, int maxWidth) {
		...
		while (left < words.length) {
			int right = rightMost(left, words, maxWidth);
	...
	private int rightMost(int left, String[] words, int maxWidth) {
		int right = left;
	...
	private int wordsLength(int left, int right, String[] words) {//the total length of words in that line
		int wordsLength = 0;
