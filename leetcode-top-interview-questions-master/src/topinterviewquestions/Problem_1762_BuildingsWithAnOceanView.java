...
Now, if the current building is strictly shorter than the last building in the answer array, then it will not block the ocean view of any of the buildings that are already present in the answer array. This is because the last building present in the array was not blocking the ocean view of any other buildings, and the current building must be even shorter. Thus, the current building will be shorter than all of the other buildings in the answer array.
...
Assume we have 66 buildings with heights, [5, 2, 1, 3, 4, 2][5,2,1,3,4,2].
Now, we can see the buildings at indices 55 and 44 do not have any next greater or equal height building to their right. Hence their views are not blocked.
The building at index 33 has the next greater or equal height building at index 44. Hence its view is blocked by the building at index 44.
Similarly, the buildings at indices 22 and 11 have their next greater or equal height building at index 33 to their right. Hence their views are blocked.
The building at index 00 does not have any next greater or equal building. Hence its view is not blocked.
...
We iterate over the given array once.
Each building's index can be pushed into and popped from the stack at most once, and both the operations take O(1)O(1) time.
The array is reversed at the end which takes O(N)O(N) time.
In Java, the copying of elements from the array list to an integer array in reverse order also takes O(N)O(N) time.
Space complexity: O(N)O(N).

An extra stack is created, which can take at most O(N)O(N) space.
In Java, in order to maintain a dynamic size array (since we don't know the size of the output array at the beginning), we created an extra Array List that supports fast O(1)O(1) push operation. The Array List may contain at most NN elements.
...
Do we really need to store all the greater or equal height buildings to the right of the current building in the stack? As we iterated over the array from right to left, we pushed each building into the stack. Each building would remain in the stack until we reached a taller building. At which point, the shorter building would be popped from the stack. This means that the tallest building seen so far would always be in the stack unless the current building is the tallest building seen so far, in which case, the stack will be empty. Simply put, while traversing from right to left, the current building will only have an ocean view if it is the tallest building seen so far.

Therefore, we can simplify the previous approach by traversing from right to left and just keep one variable to denote the tallest building seen so far. Then, if the current building's view is not blocked by the tallest building seen so far, the current building must have an ocean view. Thus, we just need to track the maximum height building encountered so far while traversing from right to left.
...
The answer array is reversed at the end, which also takes O(N)O(N) time.
In Java, copying the elements from the array list to an integer array in reverse order also takes O(N)O(N).
...
Although, in Java, in order to maintain a dynamic size array (since we don't know the size of the output array at the beginning), we created an extra Array List that supports fast O(1)O(1) push operation. Array List can contain at most NN elements, hence for the Java solution, the space complexity is O(N)O(N).
    ...
    //diss
    //Perfect problem for using a linked list
    public int[] findBuildings(int[] heights){
