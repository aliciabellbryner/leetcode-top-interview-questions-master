package topinterviewquestions;

import java.util.HashSet;
import java.util.LinkedList;
import java.util.Queue;
import java.util.Set;

/*
You are given an m x n grid where each cell can have one of three values:

0 representing an empty cell,
1 representing a fresh orange, or
2 representing a rotten orange.
Every minute, any fresh orange that is 4-directionally adjacent to a rotten orange becomes rotten.

Return the minimum number of minutes that must elapse until no cell has a fresh orange. If this is impossible, return -1.



Example 1:


Input: grid = [[2,1,1],[1,1,0],[0,1,1]]
Output: 4
Example 2:

Input: grid = [[2,1,1],[0,1,1],[1,0,1]]
Output: -1
Explanation: The orange in the bottom left corner (row 2, column 0) is never rotten, because rotting only happens 4-directionally.
Example 3:

Input: grid = [[0,2]]
Output: 0
Explanation: Since there are already no fresh oranges at minute 0, the answer is just 0.


Constraints:

m == grid.length
n == grid[i].length
1 <= m, n <= 10
grid[i][j] is 0, 1, or 2.
 */
public class Problem_0994_RottingOranges {
    //https://leetcode.com/problems/rotting-oranges/discuss/238681/Java-Clean-BFS-Solution-with-comments/337946
    //bfs
    class Solution {
        public int orangesRotting(int[][] grid) {
            //bfs, 1 find all rotten == 2
            //put 2 into a queue
            //each time poll 2 then go four directions, if next is 1, change to 2, put back to queue
            //if queue is empty, check if there is still 1

            Queue<int[]> queue = new LinkedList<>();

            for (int i = 0; i < grid.length; i++) {
                for (int j = 0; j < grid[0].length; j++) {
                    if (grid[i][j] == 2) {
                        queue.offer(new int[]{i, j});
                    }
                }
            }

            int[][] direction = {{1,0}, {0,1}, {0,-1}, {-1, 0}};

            int res = 0;
            while (!queue.isEmpty()) {
                res++;
                int size = queue.size();
                for (int i = 0; i < size; i++) {
                    int[] rotten = queue.poll();
                    for (int[] dir : direction) {
                        int nx = dir[0] + rotten[0];
                        int ny = dir[1] + rotten[1];

                        if (nx < 0 || ny < 0 || nx >= grid.length || ny >= grid[0].length || grid[nx][ny] != 1)
                            continue;

                        grid[nx][ny] = 2;
                        queue.offer(new int[]{nx, ny});
                    }
                }
            }

            for (int[] x : grid) {
                for (int y : x) {
                    if (y == 1)
                        return -1;
                }
            }

            return res == 0 ? 0 : res - 1;
        }
    }


    //DFS
    //https://leetcode.com/problems/rotting-oranges/discuss/239659/DFS-BFS-Clean-Code
    class Solution2 {
        private static final int[][] directions = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};

        public int orangesRotting(int[][] grid) {
            int numFreshOranges = 0;
            Set<Integer> curRotten = new HashSet<>();
            for (int i = 0; i < grid.length; i++)
                for (int j = 0; j < grid[0].length; j++)
                    if (grid[i][j] == 1)
                        numFreshOranges++;
                    else if (grid[i][j] == 2)
                        curRotten.add(i * grid[0].length + j);
            return rottenFrom(grid, numFreshOranges, curRotten);
        }

        private int rottenFrom(int[][] grid, int numFreshOranges, Set<Integer> curRotten) {
            if (0 == numFreshOranges) {
                return 0;
            } else if (curRotten.isEmpty()) {
                return -1;
            } else {
                Set<Integer> nextRotten = new HashSet<>();

                for (int rottenMark : curRotten) {
                    int x = rottenMark / grid[0].length, y = rottenMark % grid[0].length;
                    for (int[] direction : directions) {
                        int nx = x + direction[0], ny = y + direction[1];
                        if (nx < grid.length && nx >= 0 && ny < grid[0].length && ny >= 0 && grid[nx][ny] == 1) { // grid[nx][ny] is rotten.
                            numFreshOranges--;
                            grid[nx][ny] = 2;
                            nextRotten.add(nx * grid[0].length + ny);
                        }
                    }
                }

                int minutes = rottenFrom(grid, numFreshOranges, nextRotten);
                return minutes == -1 ? minutes : minutes + 1;
            }
        }

    }


    //DFS
    //https://leetcode.com/problems/rotting-oranges/discuss/602284/Java-DFS-No-extra-space-Beats-100
    class Solution3 {

        public int orangesRotting(int[][] grid) {
            if(grid == null || grid.length == 0) return -1;

            for(int i=0; i<grid.length; i++) {
                for(int j=0; j<grid[0].length; j++) {
                    if(grid[i][j] == 2) {
                        infect(grid, i, j, 2);
                    }
                }
            }

            int minutes = 2;
            for(int[] row : grid) {
                for(int cell : row) {
                    if(cell == 1) return -1;
                    minutes = Math.max(minutes, cell);
                }
            }

            return minutes - 2;
        }

        private void infect(int[][] grid, int i, int j, int minutes) {
            if(i < 0 || i >= grid.length /* out of bounds */
                    || j < 0 || j >= grid[0].length /* out of bounds */
                    || grid[i][j] == 0 /* empty cell */
                    || (1 < grid[i][j] && grid[i][j] < minutes) /* this orange is already rotten by another rotten orange */
            ) return;
            else {
                grid[i][j] = minutes;
                infect(grid, i - 1, j, minutes + 1);
                infect(grid, i + 1, j, minutes + 1);
                infect(grid, i, j - 1, minutes + 1);
                infect(grid, i, j + 1, minutes + 1);
            }
        }
    }
}
