 ...
 */
public class Problem_1152_AnalyzeUserWebsiteVisitPattern {
    //diss
    ...
    //Java version. Man... Python is so efficient but this is probably the most concise Java solution you're going to find in this forum.
    class Solution2 {
        public List<String> mostVisitedPattern(String[] username, int[] timestamp, String[] website) {
            List<String> compressed = new ArrayList<>();

            for (int i = 0; i < username.length; i++) {
                String serialized = timestamp[i] + "#" + username[i] + "#" + website[i];
                compressed.add(serialized);
            }

            Collections.sort(compressed, (a,b) -> Integer.parseInt(a.substring(0, a.indexOf('#')))
                    - Integer.parseInt(b.substring(0, b.indexOf('#'))));

            Map<String, List<String>> userHistory = new HashMap<>();

            for (String key : compressed) {
                String[] parsed = key.split("#");

                userHistory.putIfAbsent(parsed[1], new ArrayList<>());
                userHistory.get(parsed[1]).add(parsed[2]);
            }

            Map<String, Integer> counter = new HashMap<>();

            for (List<String> websiteList : userHistory.values()) {
                Set<String> combs = new HashSet<>();
                combinations(websiteList, combs, 0, "", 3);

                for (String comb : combs) {
                    counter.put(comb, counter.getOrDefault(comb, 0) + 1);
                }
            }
            ...
            for (Map.Entry<String, Integer> entry : counter.entrySet()) {
                if (max < entry.getValue()) {
                    ...
                    if (res == "" || res.compareTo(entry.getKey()) > 0) {
                        res = entry.getKey();
                    }
                }
            }
            ...
            return Arrays.asList(parsedRes[0], parsedRes[1], parsedRes[2]);
        }

        private void combinations(List<String> websiteList, Set<String> res, int start, String str, int k) {
            if (k == 0) {
                res.add(str);
                return;
            }

            for (int i = start; i < websiteList.size(); i++) {
                combinations(websiteList, res, i + 1, str + websiteList.get(i) + "#", k - 1);
            }
        }
    }
